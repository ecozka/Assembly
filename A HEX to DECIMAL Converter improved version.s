...
convert:                        ; converts hex to decimal and obtains ascii code
    ...
    add W6, W9, W6
    cpseq   W6, W9              ; checks if the ascii code of the first digit is equal to ascii code of 0 and skips display if it is equal
    mov.b W6, [W10++]

    add W7, W9, W7
    cpsne   W6, W9              ; to check if it is 00, first checks the first digit
    cpseq   W7,W9               ; then checks second digit if it is 00 does not display. if it is 10(first digit is nonzero, second digit is zero) displays
    mov.b W7, [W10++]

    add W8, W9, W8
    cpsne   W6, W9              ;same as before first checks the previous digits if they are all zero then controls the current digit
    cpsne   W7, W9
    cpseq   W8,W9
    mov.b W8, [W10++]

    add W13, W9, W13
    cpsne   W6, W9              ;same as before
    cpsne   W7, W9
    cpsne   W6, W9
    cpsne   W8, W9
    cpseq   W13,W9
